...
export const createExpenseSchema = z.object({
  ...
  expense_data: z.array(
    z.object({
      ...
      amount: z
        .number({ required_error: "Split amount is required" })
  ...
  const splitTotal = data.expense_data.reduce((sum, item) => sum.plus(new Prisma.Decimal(item.amount)), new Prisma.Decimal(0));
