...
export const createGroup = async (req: Request, res: Response) => {
  ...
  const { name, description, groupType, createdBy } = validGroup.data;
...
export const removeGroupMember = async (req: Request, res: Response) => {
  ...
  try {
    const groupMember = await prisma.group_members.update({
      where: { id: id },
      data: {
        isInGroup: false,
        left_at: new Date(),
    ...
    return res.status(200).json(successResponse("Group member removed successfully", groupMember));
  } catch (error) {
    // console.log(error);
